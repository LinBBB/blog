...
class RegisterView(View):
    ...
    def post(self, request):
        ...
        # redict是重定向
        # reverse是通过namespace：name来获取试图对应路由

        response = redirect(reverse("home:index"))
        # return HttpResponse("注册成功,重定向到首页")

        # 设置cokie信息,以便首页中用户信息的展示
...
class ImageCodeView(View):
    def get(self, request):
        ...
        4.将图片内容保存到redis中
            uuid作为key，图片内容作为一个value 同时我们还设置一个时效
        5.返回图片二进制
        """
        # 1.接受前端传递过来的uuid
...
class SmsCodeView(View):
    def get(self, request):
        ...
        try:
            ...
            logger.error(e)
        #         比对图片验证码,注意小写,redis的数据是字节类型
        if redis_image_code.decode().lower() != image_code.lower():
            return JsonResponse({"code": RETCODE.IMAGECODEERR, "errmsg": "图片验证码错误"})
        # 3.生成短信验证码
        sms_code = "%06d" % randint(0, 999999)
        # 为了后期比对方便，我们可以将短信记录在日志
        logger.info(sms_code)
        # 4.保存短信验证码到redis中
        redis_conn.setex("sms:%s" % mobile, 300, sms_code)
        # 5.发送短信
        CCP().send_template_sms('13679862785', [sms_code, 5], 1)
        # 6.返回相应
...
class LoginView(View):
    ...
    def post(self, request):
        ...
        if not re.match(r'^[a-zA-Z0-9]{8,20}$', password):
            return HttpResponseBadRequest('密码不合格规则')
        # 3.用户认证登录
        # 采用系统自带的认证方法进行认证
        # 如果我们的用户名和密码正确，会返回user
        # 如果我们的用户名或密码不正确，会返回None
        from django.contrib.auth import authenticate
        # 默认的认证方法是针对于username字段进行判断
...
# LoginRequiredMixin
# http://127.0.0.1:8000/accounts/login/?next=/center/
# 默认跳转
